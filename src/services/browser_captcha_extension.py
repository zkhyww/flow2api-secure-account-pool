...
EXTENSION_FINGERPRINT_FIELDS = frozenset(
    {
        "user_agent",
        "accept_language",
        "sec_ch_ua",
...
class ExtensionCaptchaService:
    ...
    async def connect(
        ...
        *,
        ...
        conn = ExtensionConnection(
            ...
            capability_marker=str(capability_marker or "").strip(),
        )
    ...
    async def handle_message(self, websocket: WebSocket, data: str):
        try:
            ...
            if message_type == "register":
                ...
                if conn:
                    debug_logger.log_info(
                        "[Extension Captcha] Client registration acknowledged "
                        f"route_connected={bool(conn.route_key)}"
                    )
                    await self._send_ack(
                        websocket,
                        {
                            ...
                            "capability_marker": conn.capability_marker,
                        },
                    )
                return
    ...
    async def get_token(
    ...
    ) -> Optional[ExtensionCaptchaBundle]:
        ...
        try:
            ...
            if result.get("status") == "success":
                captcha_token = str(result.get("token") or "").strip()
                fingerprint = result.get("fingerprint")
                if not captcha_token or not isinstance(fingerprint, dict):
                    debug_logger.log_warning(
                        "[Extension Captcha] stage=response status=rejected error_class=invalid_bundle"
                    )
                    return None
                if set(fingerprint) != EXTENSION_FINGERPRINT_FIELDS:
                    debug_logger.log_warning(
                        "[Extension Captcha] stage=response status=rejected error_class=invalid_fingerprint_fields"
                    )
                    return None
                normalized_fingerprint = {}
                for key in EXTENSION_FINGERPRINT_FIELDS:
                    value = fingerprint.get(key)
                    if not isinstance(value, str):
                        debug_logger.log_warning(
                            "[Extension Captcha] stage=response status=rejected error_class=invalid_fingerprint_value"
                        )
                        return None
                    normalized_fingerprint[key] = value.strip()
                return ExtensionCaptchaBundle(
                    token=captcha_token,
